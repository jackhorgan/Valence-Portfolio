...
function AppWithSnapshot(){
 ...
 const currency=state.currency;
 useEffect(()=>{let cancelled=false; async function fetchFX(){try{const r=await fetch(FX_API,{cache:'no-store'});if(!r.ok)throw new Error('FX request failed');const d=await r.json();if(!cancelled&&d?.rate){setFxRate(Number(d.rate));setFxDate(d.date);}}catch(e){console.error('Valence FX update failed',e);}} fetchFX(); return()=>{cancelled=true}},[]);
