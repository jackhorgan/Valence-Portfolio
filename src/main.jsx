...
function Transactions({state,currency,setModal,updateState}){return <><div className="page-head"><div><div className="eyebrow">JOURNAL</div><h1>Transactions</h1><p>Log buys, sells, dividends and cash movements.</p></div><button className="outline-btn" onClick={()=>setModal('transaction')}><Plus size={16}/> Add transaction</button></div><section className="panel"><div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Ticker</th><th>Quantity</th><th>Price</th><th>Total</th><th>Notes</th><th></th></tr></thead><tbody>{state.transactions.map(t=>{const total=t.type==='Div'?t.fee:t.quantity*t.price+t.fee;return <tr key={t.id}><td>{t.date}</td><td><span className={`badge ${t.type.toLowerCase()}`}>{t.type}</span></td><td><b>{t.ticker}</b></td><td>{t.quantity||'—'}</td><td>{t.price?fmtMoney(t.price,currency):'—'}</td><td>{fmtMoney(total,currency)}</td><td className="notes">{t.notes||'—'}</td><td><button className="danger-mini" onClick={()=>updateState({...state,transactions:state.transactions.filter(x=>x.id!==t.id)})}><Trash2 size={15}/></button></td></tr>})}</tbody></table></div></section></>}
...
function TransactionModal({state,currency,onClose,onSave}){const [f,setF]=useState({date:new Date().toISOString().slice(0,10),type:'Buy',ticker:'',quantity:'',price:'',fee:'',notes:''});const set=(k,v)=>setF({...f,[k]:v});return <Modal title="Add transaction" onClose={onClose} footer={<><button className="ghost-btn" onClick={onClose}>Cancel</button><button className="primary-btn" onClick={()=>{if(!f.ticker)return;onSave({...f,id:crypto.randomUUID(),quantity:Number(f.quantity||0),price:Number(f.price||0),fee:Number(f.fee||0)})}}>Save transaction</button></>}><div className="form-grid"><Field label="Date"><input type="date" value={f.date} onChange={e=>set('date',e.target.value)}/></Field><Field label="Type"><select value={f.type} onChange={e=>set('type',e.target.value)}><option>Buy</option><option>Sell</option><option>Div</option></select></Field><Field label="Ticker"><input placeholder="NVDA" value={f.ticker} onChange={e=>set('ticker',e.target.value.toUpperCase())}/></Field><Field label="Quantity"><input type="number" step="any" value={f.quantity} onChange={e=>set('quantity',e.target.value)}/></Field><Field label="Price per share"><input type="number" step="any" value={f.price} onChange={e=>set('price',e.target.value)}/></Field><Field label="Fee"><input type="number" step="any" value={f.fee} onChange={e=>set('fee',e.target.value)}/></Field><Field label="Notes / strategy"><textarea value={f.notes} onChange={e=>set('notes',e.target.value)} placeholder="Weekly DCA contribution"/></Field></div></Modal>}
