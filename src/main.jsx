...
const fmtNZD = n => new Intl.NumberFormat('en-NZ',{style:'currency',currency:'NZD',maximumFractionDigits:2}).format(n || 0);
const fmtFX = n => n ? Number(n).toFixed(4) : '—';
async function fetchUsdNzd(){
  try{
    const r=await fetch('https://api.frankfurter.app/latest?from=USD&to=NZD',{cache:'no-store'});
    if(!r.ok) throw new Error('FX request failed');
    const d=await r.json();
    const rate=Number(d?.rates?.NZD);
    if(!rate) throw new Error('No USD/NZD rate returned');
    return {rate, date:d.date, source:'Frankfurter / ECB reference rates'};
  }catch{return null}
}

function loadState(){ try { const raw=localStorage.getItem(STORAGE); return raw?JSON.parse(raw):{holdings:demoHoldings,transactions:demoTransactions,snapshots:demoSnapshots,cash:1240,currency:'USD',fxRate:null,fxUpdated:null,fxSource:null}; } catch { return {holdings:demoHoldings,transactions:demoTransactions,snapshots:demoSnapshots,cash:1240,currency:'USD',fxRate:null,fxUpdated:null,fxSource:null}; } }
function App(){
 ...
 const reset=()=>{if(confirm('Reset to the demo portfolio?')) updateState({holdings:demoHoldings,transactions:demoTransactions,snapshots:demoSnapshots,cash:1240,currency:'USD',fxRate:null,fxUpdated:null,fxSource:null});};
 return <div className="app">
  ...
  <main className="main">
   ...
   <div className="content">
    {page==='dashboard' && <Dashboard {...{state,positions,totalValue,totalInvested,unrealized,perfPct,annualIncome,dayMove,allocation,chartData,currency,setModal}}/>}
    {page==='holdings' && <Holdings {...{state,positions,totalValue,currency,updateState,setModal}}/>}
  ...
  {modal==='transaction' && <TransactionModal state={state} currency={currency} onClose={()=>setModal(null)} onSave={(tx)=>{updateState({...state,transactions:[tx,...state.transactions]});setModal(null)}}/>}
...
function Dashboard(p){
 ...
 return <>
  ...
  <section className="panel table-panel"><div className="panel-head"><div><h2>Open holdings</h2><span>Current portfolio positions</span></div><button className="link-btn" onClick={()=>p.setModal('holding')}>View all →</button></div><div className="table-scroll"><table><thead><tr><th>Position</th><th>Price</th><th>Value</th><th>P/L</th><th>Weight</th></tr></thead><tbody>{p.allocation.slice(0,6).map(x=><tr key={x.id}><td><div className="ticker"><span>{x.ticker}</span><small>{x.name}</small></div></td><td>{fmtMoney(x.price,p.currency)}</td><td>{fmtMoney(x.value,p.currency)}</td><td className={x.pl>=0?'positive':'negative'}>{x.pl>=0?'+':''}{fmtMoney(x.pl,p.currency)}<small>{fmtPct(x.plPct)}</small></td><td><div className="weight"><div className="bar"><i style={{width:`${Math.min(x.weight,100)}%`}}></i></div><span>{x.weight.toFixed(1)}%</span></div></td></tr>)}</tbody></table></div></section>
  <div className="quick-grid"><button onClick={()=>p.setModal('transaction')}><Plus size={18}/><div><b>Add transaction</b><span>Buy, sell, dividend or deposit</span></div></button><button onClick={()=>p.setModal('holding')}><Wallet size={18}/><div><b>Add a position</b><span>Set up a stock or crypto holding</span></div></button><button onClick={()=>p.setModal('snapshot')}><CalendarDays size={18}/><div><b>Log weekly snapshot</b><span>Capture your end-of-week value</span></div></button></div>
 </>
}
function Holdings({state,positions,totalValue,currency,updateState,setModal}){return <><div className="page-head"><div><div className="eyebrow">POSITIONS</div><h1>Open holdings</h1><p>Everything you currently own in one place.</p></div><button className="outline-btn" onClick={()=>setModal('holding')}><Plus size={16}/> Add position</button></div><section className="panel"><div className="table-scroll"><table><thead><tr><th>Position</th><th>Shares</th><th>Avg cost</th><th>Current price</th><th>Value</th><th>P/L</th><th>Weight</th><th></th></tr></thead><tbody>{positions.map(x=><tr key={x.id}><td><div className="ticker"><span>{x.ticker}</span><small>{x.name} · {x.sector}</small></div></td><td>{x.shares}</td><td>{fmtMoney(x.avgCost,currency)}</td><td>{fmtMoney(x.price,currency)}</td><td>{fmtMoney(x.value,currency)}</td><td className={x.pl>=0?'positive':'negative'}>{x.pl>=0?'+':''}{fmtMoney(x.pl,currency)}<small>{fmtPct(x.plPct)}</small></td><td>{totalValue?`${(x.value/totalValue*100).toFixed(1)}%`:'0%'}</td><td><button className="danger-mini" onClick={()=>{if(confirm(`Delete ${x.ticker}?`)) updateState({...state,holdings:state.holdings.filter(h=>h.id!==x.id)})}}><Trash2 size={15}/></button></td></tr>)}</tbody></table></div></section></>}
function Transactions({state,currency,setModal,updateState}){
 return <><div className="page-head"><div><div className="eyebrow">JOURNAL</div><h1>Transactions</h1><p>Record what happened in Sharesies, including the actual NZD conversion.</p></div><button className="outline-btn" onClick={()=>setModal('transaction')}><Plus size={16}/> Add transaction</button></div>
 <section className="panel"><div className="panel-head"><div><h2>Transaction history</h2><span>USD trades + actual NZD paid/received</span></div></div><div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Ticker</th><th>Quantity</th><th>USD</th><th>NZD actual</th><th>Broker FX</th><th>Fees</th><th></th></tr></thead><tbody>
 {state.transactions.map(t=>{const usd=t.usdValue ?? ((t.quantity||0)*(t.price||0)); return <tr key={t.id}><td>{t.date}</td><td><span className={`badge ${t.type.toLowerCase()}`}>{t.type==='Div'?'Dividend':t.type}</span></td><td><b>{t.ticker||'—'}</b></td><td>{t.quantity||'—'}</td><td>{usd?fmtMoney(usd,'USD'):'—'}</td><td>{t.nzdValue?fmtNZD(t.nzdValue):'—'}</td><td>{t.fxRate?fmtFX(t.fxRate):'—'}</td><td>{t.feeUsd?fmtMoney(t.feeUsd,'USD'):''}{t.fxFeeNzd?` + ${fmtNZD(t.fxFeeNzd)}`:''}</td><td><button className="danger-mini" onClick={()=>updateState({...state,transactions:state.transactions.filter(x=>x.id!==t.id)})}><Trash2 size={15}/></button></td></tr>})}
...
function TransactionModal({onClose,onSave}){
 const [f,setF]=useState({date:new Date().toISOString().slice(0,10),type:'Buy',ticker:'',quantity:'',price:'',usdValue:'',nzdValue:'',fxRate:'',feeUsd:'',fxFeeNzd:'',notes:''});
 const set=(k,v)=>setF({...f,[k]:v});
 const trade=['Buy','Sell'].includes(f.type), dividend=f.type==='Dividend', cashMove=['Deposit','Withdrawal'].includes(f.type);
 const calculatedUsd=trade ? (Number(f.quantity||0)*Number(f.price||0)) : Number(f.usdValue||0);
 const calculatedFx=(Number(f.nzdValue||0)&&calculatedUsd)?Number(f.nzdValue)/calculatedUsd:0;
 return <Modal title="Add transaction" onClose={onClose} footer={<>
   <button className="ghost-btn" onClick={onClose}>Cancel</button>
   <button className="primary-btn" onClick={()=>{
     if((trade||dividend)&&!f.ticker.trim())return alert('Enter a ticker.');
     if(trade&&(!Number(f.quantity)||!Number(f.price)))return alert('Enter quantity and USD price.');
     if(dividend&&!Number(f.nzdValue||f.usdValue))return alert('Enter the dividend amount.');
     if(cashMove&&!Number(f.nzdValue))return alert('Enter the NZD amount.');
     onSave({...f,id:crypto.randomUUID(),type:f.type,quantity:Number(f.quantity||0),price:Number(f.price||0),
       usdValue:calculatedUsd,nzdValue:Number(f.nzdValue||0),fxRate:Number(f.fxRate||calculatedFx||0),
       feeUsd:Number(f.feeUsd||0),fxFeeNzd:Number(f.fxFeeNzd||0),notes:f.notes||''});
   }}>Save transaction</button>
 </>}>
 <div className="form-grid">
  <Field label="Date"><input type="date" value={f.date} onChange={e=>set('date',e.target.value)}/></Field>
  <Field label="Type"><select value={f.type} onChange={e=>set('type',e.target.value)}><option>Buy</option><option>Sell</option><option>Dividend</option><option>Deposit</option><option>Withdrawal</option></select></Field>
  <Field label="Ticker"><input placeholder="NVDA" value={f.ticker} disabled={cashMove} onChange={e=>set('ticker',e.target.value.toUpperCase())}/></Field>
  {trade&&<><Field label="Quantity / shares"><input type="number" step="any" value={f.quantity} onChange={e=>set('quantity',e.target.value)}/></Field>
  <Field label="Price per share (USD)"><input type="number" step="any" value={f.price} onChange={e=>set('price',e.target.value)}/></Field></>}
  <Field label={trade?'USD trade value':dividend?'Dividend (USD)':'USD amount'}><input type="number" step="any" value={f.usdValue||calculatedUsd||''} onChange={e=>set('usdValue',e.target.value)} disabled={trade}/></Field>
  <Field label="NZD amount actually paid / received"><input type="number" step="any" value={f.nzdValue} onChange={e=>set('nzdValue',e.target.value)} placeholder="Copy the actual NZD amount from Sharesies"/></Field>
  {(trade||dividend)&&<Field label="Broker FX rate"><input type="number" step="any" value={f.fxRate} onChange={e=>set('fxRate',e.target.value)} placeholder={calculatedFx?String(calculatedFx.toFixed(4)):'e.g. 1.6800'}/></Field>}
  {(trade||dividend)&&<Field label="Sharesies fee (USD)"><input type="number" step="any" value={f.feeUsd} onChange={e=>set('feeUsd',e.target.value)} placeholder="0"/></Field>}
  {(trade||dividend)&&<Field label="FX fee (NZD)"><input type="number" step="any" value={f.fxFeeNzd} onChange={e=>set('fxFeeNzd',e.target.value)} placeholder="0"/></Field>}
  <Field label="Notes / strategy"><textarea value={f.notes} onChange={e=>set('notes',e.target.value)} placeholder="Weekly DCA contribution"/></Field>
 </div>
 <div className="auth-message" style={{marginTop:12}}>Sharesies: enter the <b>actual NZD amount</b> you paid/received. Valence stores your historical broker FX rate separately from today's NZD valuation.</div>
 </Modal>
}
...
function AppWithSnapshot(){
 ...
 useEffect(()=>{
   let cancelled=false;
   fetchUsdNzd().then(fx=>{
     if(!cancelled && fx) setState(prev=>({...prev,fxRate:fx.rate,fxUpdated:fx.date,fxSource:fx.source}));
   });
  ...
  try{
   ...
   if(hasCloud){
    const next={
     ...
     cash:1240,currency:state.currency||'USD',fxRate:state.fxRate||null,fxUpdated:state.fxUpdated||null,fxSource:state.fxSource||null
    };
 ...
 async function syncCloud(next,userId=session?.user?.id){
  ...
  try{
   ...
   if(next.transactions.length) jobs.push(supabase.from('transactions').insert(next.transactions.map(t=>({user_id:userId,transaction_date:t.date,transaction_type:t.type==='Dividend'?'Div':t.type,ticker:t.ticker||null,quantity:t.quantity||0,price_per_share:t.price||0,fee:t.feeUsd||t.fee||0,total_value:t.usdValue||((t.quantity||0)*(t.price||0)),usd_value:t.usdValue||0,nzd_value:t.nzdValue||0,fx_rate:t.fxRate||0,fee_usd:t.feeUsd||t.fee||0,fx_fee_nzd:t.fxFeeNzd||0,notes:t.notes||''}))));
 ...
 return <div className="app"><aside className={`sidebar ${menuOpen?'open':''}`}><div className="brand"><div className="brandmark">V</div><div><div className="brandname">VALENCE</div><div className="brandtag">PORTFOLIO OS</div></div></div><nav><NavItem icon={<LayoutDashboard size={18}/>} label="Dashboard" active={page==='dashboard'} onClick={()=>nav('dashboard')}/><NavItem icon={<Wallet size={18}/>} label="Holdings" active={page==='holdings'} onClick={()=>nav('holdings')}/><NavItem icon={<ClipboardList size={18}/>} label="Transactions" active={page==='transactions'} onClick={()=>nav('transactions')}/><NavItem icon={<CalendarDays size={18}/>} label="Reports" active={page==='reports'} onClick={()=>nav('reports')}/></nav><div className="sidebar-bottom"><label className="side-link file-link"><Upload size={18}/>Import backup<input type="file" accept="application/json" onChange={e=>e.target.files[0]&&importJSON(e.target.files[0])}/></label><button className="side-link" onClick={saveJSON}><Download size={18}/>Export backup</button><button className="side-link" onClick={()=>{if(confirm('Reset to demo data?'))updateState({holdings:demoHoldings,transactions:demoTransactions,snapshots:demoSnapshots,cash:1240,currency:'USD',fxRate:null,fxUpdated:null,fxSource:null})}}><RefreshCw size={18}/>Reset demo</button><button className="side-link" onClick={signOut}><X size={18}/>Sign out</button><div className="sync-card"><ShieldCheck size={17}/><div><b>{syncing?'Syncing…':'Cloud synced'}</b><span>Portfolio saved to your Valence account.</span></div></div></div></aside><main className="main"><header className="topbar"><button className="iconbtn menu-btn" onClick={()=>setMenuOpen(!menuOpen)}><Menu size={20}/></button><div className="crumb"><span>Portfolio</span><ChevronDown size={15}/><b>{page[0].toUpperCase()+page.slice(1)}</b></div><div className="top-actions"><div className="date-pill"><Activity size={15}/> Week of Sep 15, 2026</div><button className="iconbtn"><Bell size={18}/></button><button className="add-btn" onClick={()=>setModal('transaction')}><Plus size={17}/>Add</button></div></header><div className="content">{page==='dashboard'&&<><section className="panel fx-panel"><div className="panel-head"><div><h2>NZD overview</h2><span>Current USD portfolio translated using the latest reference FX rate</span></div><span className="badge div">{state.fxRate?'FX AVAILABLE':'FX UNAVAILABLE'}</span></div><div className="kpis"><div className="kpi"><span>Portfolio USD</span><b>{fmtMoney(totalValue,'USD')}</b></div><div className="kpi"><span>Portfolio NZD</span><b>{state.fxRate?fmtNZD(totalValue*state.fxRate):'—'}</b></div><div className="kpi"><span>USD/NZD</span><b>{state.fxRate?fmtFX(state.fxRate):'—'}</b></div></div><div className="note">{state.fxRate?`Rate date: ${state.fxUpdated||'latest available'} · ${state.fxSource||'reference source'}`:'Current NZD valuation is unavailable until the FX provider responds. Valence will not substitute a guessed rate.'}</div></section><Dashboard state={state} positions={positions} totalValue={totalValue} totalInvested={totalInvested} unrealized={unrealized} perfPct={perfPct} annualIncome={annualIncome} dayMove={0} allocation={allocation} chartData={state.snapshots} currency={currency} setModal={m=>m==='snapshot'?setSnapshotOpen(true):setModal(m)}/></>} {page==='holdings'&&<Holdings state={state} positions={positions} totalValue={totalValue} currency={currency} updateState={updateState} setModal={setModal}/>} {page==='transactions'&&<Transactions state={state} currency={currency} setModal={setModal} updateState={updateState}/>} {page==='reports'&&<Reports state={state} currency={currency} positions={positions} totalValue={totalValue} totalInvested={totalInvested} unrealized={unrealized} allocation={allocation} chartData={state.snapshots} setModal={m=>m==='snapshot'?setSnapshotOpen(true):setModal(m)} updateState={updateState}/>}</div></main>{modal==='transaction'&&<TransactionModal state={state} currency={currency} onClose={()=>setModal(null)} onSave={tx=>{
 const next={...state,transactions:[tx,...state.transactions]};
 if(tx.type==='Buy'){
   const h=next.holdings.find(h=>h.ticker===tx.ticker);
   if(h){const oldCost=h.shares*h.avgCost;const added=tx.usdValue+(tx.feeUsd||0);h.shares+=tx.quantity;h.avgCost=h.shares?(oldCost+added)/h.shares:h.avgCost;h.price=tx.price;}
   else next.holdings=[...next.holdings,{id:crypto.randomUUID(),ticker:tx.ticker,name:tx.ticker,assetType:'Stock',sector:'Unknown',shares:tx.quantity,avgCost:(tx.usdValue+(tx.feeUsd||0))/(tx.quantity||1),price:tx.price,dividendYield:0}];
 }
 if(tx.type==='Sell'){const h=next.holdings.find(h=>h.ticker===tx.ticker);if(h)h.shares=Math.max(0,h.shares-tx.quantity);h&& (h.price=tx.price);}
 updateState(next);setModal(null)
}}/>}{modal==='holding'&&<HoldingModal onClose={()=>setModal(null)} onSave={h=>{updateState({...state,holdings:[...state.holdings,h]});setModal(null)}}/>}{snapshotOpen&&<SnapshotModal onClose={()=>setSnapshotOpen(false)} onSave={s=>{updateState({...state,snapshots:[...state.snapshots,s].sort((a,b)=>a.date.localeCompare(b.date))});setSnapshotOpen(false)}}/>}</div>
}
